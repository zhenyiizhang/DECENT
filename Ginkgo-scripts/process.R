...
maxPloidy   = 2.0
minBinWidth = 5
main_dir="/lustre/home/2301110060/ginkgo-master/scripts/"
...
# Normalize cells
normal  = sweep(raw+1, 2, colMeans(raw+1), '/')####(data+1)/colMeans(data+1)
normal2 = normal
lab     = colnames(normal)
...
for(k in 1:w)
{
  ...
  if (stat == 0) {
    ...
    lr = log2((normal[,k])/(F))
    normal[,k] = normal[,k]/(F) ###key ref
dim_normk <- dim(normal)


file_path <- "/lustre/home/2301110060/ginkgo-master/scripts/dim.txt"


file_conn <- file(file_path, "w")


cat("Dimensions of 'normal' matrix:\n", file = file_conn)
cat(paste("Number of rows: ", dim_normk[1], "\n"), file = file_conn)
cat(paste("Number of columns: ", dim_normk[2], "\n\n"), file = file_conn)


close(file_conn)


print("Dimensions information has been saved to dimensions_info.txt")
  ...
  segs         = segment(CNA.smoothed, verbose=0, min.width=minBinWidth,alpha = 0.0001, undo.prune=0.05)###key
...
segment_mean <- mean(normal[,k][1:bps[2]])
segment_sd <- sd(normal[,k][1:bps[2]])

if (segment_sd != 0) {
  normal[,k][1:bps[2]] <- (normal[,k][1:bps[2]] - segment_mean) * sqrt(0.5) + segment_mean
} else {
  normal[,k][1:bps[2]] <- normal[,k][1:bps[2]]
}

for(i in 2:(len-1)) {
  
  segment_mean <- mean(normal[,k][bps[i]:(bps[i+1]-1)])
  segment_sd <- sd(normal[,k][bps[i]:(bps[i+1]-1)])
  
  
  if (segment_sd != 0) {
    normal[,k][bps[i]:(bps[i+1]-1)] <- (normal[,k][bps[i]:(bps[i+1]-1)] - segment_mean) * sqrt(0.5) + segment_mean
  } else {
    
    normal[,k][bps[i]:(bps[i+1]-1)] <- normal[,k][bps[i]:(bps[i+1]-1)]
  }
}
 ...
 # jpeg(filename=paste(lab[k], "_SoS.jpeg", sep=""), width=2500, height=1500)
  #pdf(file = "CNV.pdf", height=1.7, width=6)
  ...
  #pdf(file = paste(lab[k], "_CN.pdf", sep=""), height=2, width=6)##height=1.7
  #top=4###8
  #rectangles1=data.frame(pos[seq(1,nrow(pos), 2),])
  #rectangles=c(t(rectangles1))
  #rectangles2=data.frame(pos[seq(2,nrow(pos), 2),])
  #clouds1=data.frame(x=1:l, y=normal[,k]*CN)
  #clouds=clouds1[c(1:2692), ]
  #amp=data.frame(x=which(final[c(1:2692),k]>2), y=final[which(final[c(1:2692),k]>2),k])
  #del=data.frame(x=which(final[c(1:2692),k]<2), y=final[which(final[c(1:2692),k]<2),k])
  #flat=data.frame(x=which(final[c(1:2692),k]==2), y=final[which(final[c(1:2692),k]==2),k])
  #anno=data.frame(x=(pos[,2]+pos[,1])/2, y=-top*.05, chrom=substring(c(as.character(bounds[,1]), "chrY"), 4 ,5))

  #plot1 = ggplot() +
   # geom_rect(data=rectangles1, aes(xmin=X1, xmax=X2, ymin=-top*.1, ymax=top), fill='gray85', alpha=0.75) +
   # geom_rect(data=rectangles2, aes(xmin=X1, xmax=X2, ymin=-top*.1, ymax=top), fill='gray75', alpha=0.75) +
   # geom_point(data=clouds, aes(x=x, y=y), color='gray45', size=0.2) +
   # geom_point(data=flat, aes(x=x, y=y), size=0.2) +
   # geom_point(data=amp, aes(x=x, y=y), size=0.2, color=colors[cp,1]) +
   # geom_point(data=del, aes(x=x, y=y), size=0.2, color=colors[cp,2]) +
    #geom_text(data=anno, aes(x=x, y=y, label=chrom), size=12) +
   # scale_x_continuous(limits=c(0, l), expand = c(0, 0)) +
   # scale_y_continuous(limits=c(-top*.1, top), expand = c(0, 0)) +
   # labs(title=lab[k], x="Chromosome", y="Copy Number", size=4) +
    ...
    #theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.line.x = element_blank()) +
    #theme(panel.background = element_rect(fill = 'gray90')) +
    #theme(plot.margin=unit(c(.5,1,.5,1),"cm")) +theme_bw()+
    #theme(panel.grid=element_blank(),axis.text.x=element_text(color="white", size=40),axis.text.y=element_text(color="black", size=4), axis.ticks.x=element_line(color="white"),axis.title.x=element_text(size=4, vjust=-.05,color="white"), axis.title.y=element_text(size=4, vjust=.1)) +
  #                                                                                                                                                                          geom_vline(xintercept = c(1, l), size=.5,colour="#77797b") +
 #                                                                                                                                                                           geom_hline(yintercept = c(-top*.1, top), size=.5,colour="#77797b")+
  #                                                                                                                                                                          geom_vline(xintercept = rectangles,colour="black", linetype = "longdash",size=0.1)
...
#write.table(amp,file = "amp.csv",sep = ",")
#write.table(del,file = "del.csv",sep = ",")
#write.table(flat,file = "flat.csv",sep = ",")
...
pdf(file = paste(lab[k], "_CN.pdf", sep=""), height=1.7, width=6)##height=1.7
  top=4###8
  rectangles1=data.frame(pos[seq(1,nrow(pos), 2),])
  rectangles=c(t(rectangles1))
  #rectangles2=data.frame(pos[seq(2,nrow(pos), 2),])
...
#  flat=data.frame(x=which(final[c(1:2692),k]==2), y=final[which(final[c(1:2692),k]==2),k])
  ...
  plot1 = ggplot() +
    # geom_rect(data=rectangles1, aes(xmin=X1, xmax=X2, ymin=-top*.1, ymax=top), fill='gray85', alpha=0.75) +
    # geom_rect(data=rectangles2, aes(xmin=X1, xmax=X2, ymin=-top*.1, ymax=top), fill='gray75', alpha=0.75) +
    geom_point(data=clouds, aes(x=x, y=y), color='gray45', size=0.1) +
    geom_point(data=flat, aes(x=x, y=y), size=0.1) +
    geom_point(data=amp, aes(x=x, y=y), size=0.2, color=colors[cp,1]) +
    geom_point(data=del, aes(x=x, y=y), size=0.2, color=colors[cp,2]) +
    #geom_text(data=anno, aes(x=x, y=y, label=chrom), size=12) +
    scale_x_continuous(limits=c(0, l), expand = c(0, 0)) +
...
write.table(cbind(loc2,clouds), file=paste(user_dir, "/clouds", sep=""), sep="\t", quote=FALSE, col.names=c(colnames(loc2,lab)))

# ------------------------------------------------------------------------------
# -- calculate chr copy distribution --edit by CYD
# ------------------------------------------------------------------------------
####calculate copy number length####
sample_used_B1_SegCopy <- read.table("SegCopy",header = TRUE,sep = "\t")
sample_used_B1_SegCopy.use <- sample_used_B1_SegCopy[c(1:2554), ]####1M division

##########paste(chr,copynum,sep=".")
for (i in 4:ncol(sample_used_B1_SegCopy.use)){
  sample_used_B1_SegCopy.use[ ,i] <- paste(sample_used_B1_SegCopy.use$CHR,sample_used_B1_SegCopy.use[ ,i],sep = ".")
}

###########Count the number of consecutive lines in which a character appears

for (i in 4:ncol(sample_used_B1_SegCopy.use)){
  
  sample_used_B1_SegCopy.use.count <- rle(sample_used_B1_SegCopy.use[ ,i])###statis segment length
  sample_used_B1_SegCopy.use.count.statis <- data.frame(sample=colnames(sample_used_B1_SegCopy.use)[i],values=sample_used_B1_SegCopy.use.count$values,lengths=sample_used_B1_SegCopy.use.count$lengths)
  file = paste0( colnames(sample_used_B1_SegCopy.use)[i],"_SegCopy.txt")
  write.table(sample_used_B1_SegCopy.use.count.statis,file,quote=FALSE,col.names = TRUE,row.names = FALSE)
...
for(i in 4:ncol(sample_used_B1_SegCopy.use)){
  file = paste0( colnames(sample_used_B1_SegCopy.use)[i],"_SegCopy.txt")
  sample_used_B1_SegCopy.use.count.statis.new <- read.table(file, header =TRUE)
  l <- rbind(l, sample_used_B1_SegCopy.use.count.statis.new)
}

write.table(l,"Chr_copy_statistics.xls",quote = FALSE,col.names = TRUE,row.names = FALSE,sep = "\t")
####calculate copy number location####
####calculate copy number location####
colnames(sample_used_B1_SegCopy.use)[1:3] <- c("chrom", "start", "end")
####input cytoband####
cytoband <- read.table("/gpfs1/tangfuchou_pkuhpc/tangfuchou_cls/chenyidong/project/GY/04.plot/CNV/Gingko/ginkgo-master/scripts/hg19_cytobandTables.txt",header = T)
colnames(cytoband) <- c("chrom", "start", "end","location")
library(valr)
library(dplyr)
###########Count the number of consecutive lines in which a character appears
for (i in 4:ncol(sample_used_B1_SegCopy.use)){
  sample_used_B1_SegCopy.use1 <- sample_used_B1_SegCopy.use[ ,c(1:3,i)]
  sample_used_B1_SegCopy.use2 <- sample_used_B1_SegCopy.use1[which(sample_used_B1_SegCopy.use1[ ,4] %in% c("1","3")), ]
  sample_used_B1_SegCopy.use.cytoband.statis <- bed_intersect(cytoband, sample_used_B1_SegCopy.use2)
  sample_used_B1_SegCopy.use.cytoband.statis$start.y <- NULL
  sample_used_B1_SegCopy.use.cytoband.statis$end.y <- NULL
  sample_used_B1_SegCopy.use.cytoband.statis$.overlap <- NULL
  colnames(sample_used_B1_SegCopy.use.cytoband.statis)[1:4] <- c("chrom", "start", "end","location")
  colnames(sample_used_B1_SegCopy.use.cytoband.statis)[5] <- colnames(sample_used_B1_SegCopy.use)[i]
  sample_used_B1_SegCopy.use.cytoband.statis$name <- paste(sample_used_B1_SegCopy.use.cytoband.statis$chrom,sample_used_B1_SegCopy.use.cytoband.statis$start,sample_used_B1_SegCopy.use.cytoband.statis$end,sample_used_B1_SegCopy.use.cytoband.statis$location,sep = "_")
  sample_used_B1_SegCopy.use.cytoband.statis.new <- sample_used_B1_SegCopy.use.cytoband.statis[which(!duplicated(sample_used_B1_SegCopy.use.cytoband.statis$name)), ]
  sample_used_B1_SegCopy.use.cytoband.statis.new$name <- NULL
  file = paste0( colnames(sample_used_B1_SegCopy.use)[i],"_SegCopy_cytoband.txt")
  write.table(sample_used_B1_SegCopy.use.cytoband.statis.new,file,quote=FALSE,col.names = TRUE,row.names = FALSE,sep="\t")
  
  
}

cytoband_txt <- paste0(cytoband$chrom,":",cytoband$start,"_",cytoband$end,"_",cytoband$location)
all_sample_cytoband <- data.frame(loci=cytoband_txt)
#sample_used_B1_SegCopy.input <- sample_used_B1_SegCopy.use[ ,c(4:ncol(sample_used_B1_SegCopy.use))]
library(reshape2)
for(i in 4:ncol(sample_used_B1_SegCopy.use)){
  file = paste0( colnames(sample_used_B1_SegCopy.use)[i],"_SegCopy_cytoband.txt")
  sample_cytoband <- read.table(file, header =TRUE)
  if(nrow(sample_cytoband) !=0){
  sample_cytoband$loci <- paste0(sample_cytoband$chrom,":",sample_cytoband$start,"_",sample_cytoband$end,"_",sample_cytoband$location)
  sample_cytoband$chrom <- NULL
  sample_cytoband$start <- NULL
  sample_cytoband$end <- NULL
  sample_cytoband$location <- NULL
  all_sample_cytoband <- merge(all_sample_cytoband, sample_cytoband,
                           by = c("loci"), all = T)
  }
}
order <- as.vector(cytoband_txt)
rownames(all_sample_cytoband) <- all_sample_cytoband$loci
all_sample_cytoband.order1 <- all_sample_cytoband[order, ]
all_sample_cytoband.new <- cbind(cytoband,all_sample_cytoband.order1)
all_sample_cytoband.new$loci <- NULL

write.table(all_sample_cytoband.new,"all_sample_cytoband.xls",quote = FALSE,col.names = TRUE,row.names = FALSE,sep = "\t")
